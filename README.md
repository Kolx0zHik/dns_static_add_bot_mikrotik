...
Бот добавляет DNS Static записи типа FWD на MikroTik через SSH. Поддерживает несколько роутеров, разграничение доступа по Telegram ID для каждого MikroTik, главное inline-меню, сценарий `/add` с проверкой домена и обязательным подтверждением.
...
После `/start` бот показывает только те MikroTik, к которым у пользователя есть доступ. Если доступен один роутер, бот сразу открывает меню действий. Если доступно несколько, пользователь сначала выбирает роутер, а затем добавляет DNS FWD запись с подтверждением.
